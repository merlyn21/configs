...
#psql -h 192.168.156.121 -p 49153 -U postgres -W
db_host = 'localhost'
db_name = 'vmware'
db_user = 'postgres'
db_password = ''
...
for edge_gateway in edge_gateways:
    ...
    # Get IP addresses from NAT rules
    ...
    # Get IP addresses from Firewall rules
...
delete_query = sql.SQL("DELETE FROM {} WHERE provider = 'DATALINE' AND ip NOT IN ({})").format(
...
for ip in filtered_ips:
    cur.execute(sql.SQL("INSERT INTO {} (provider, org_name, vdc_name, ip) VALUES (%s, %s, %s, %s) ON CONFLICT (ip) DO NOTHING").format(sql.Identifier(table_name)), [provider, org_name, vdc_name, ip])
